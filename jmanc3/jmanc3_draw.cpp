...
draw_original_4coder_style_cursor_mark_highlight(Application_Links *app, View_ID view_id, b32 is_active_view,
    ...
    FColor cursor_color = f_black;
    
    Managed_Scope scope = buffer_get_managed_scope(app, buffer);
    Command_Map_ID* map_id_ptr = scope_attachment(app, scope, buffer_map_id, Command_Map_ID);
    
    if (*map_id_ptr == (i64) mapid_insert) {
        cursor_color = fcolor_id(defcolor_cursor);
    } else if (*map_id_ptr == (i64) mapid_modal) {
        cursor_color = f_red;
    } else if (*map_id_ptr == (i64) mapid_global) {
        cursor_color = f_blue;
    }
